...
public class StablishmentController {
    ...
    public ResponseEntity<byte[]> exportToExcel(
        ...
        List<StablishmentDTO> data = "deleted".equalsIgnoreCase(type)
                ? service.listDeleted()
                : service.listActive();
        ...
        try (Workbook wb = new XSSFWorkbook()) {
            ...
            // === Datos ===
            int rowIdx = 1;
            for (StablishmentDTO u : data) {
                Row row = sheet.createRow(rowIdx++);
                int col = 0;

                row.createCell(col++).setCellValue(u.getId());
                row.createCell(col++).setCellValue(nvl(u.getName()));
        ...
        String filename = "Establecimientos_" + ("deleted".equalsIgnoreCase(type) ? "Eliminados" : "Activos") + ".xlsx";
